...
class TestDictionaries(unittest.TestCase):
    def test_dictionary_creation(self):
        ...
        self.assertEqual(dictionaries.a_dict(), dictionaries.another_dict())
    ...
    def test_create_dictionary_fromkeys(self):
        self.assertEqual(
            dict.fromkeys(["basic", "crest"]), {"basic": None, "crest": None}
        )
    ...
    def test_listing_dictionary_values(self):
        self.assertEqual(list(dictionaries.a_dict().values()), ["value"])
    ...
    def test_how_to_get_value_when_key_does_not_exist(self):
        self.assertIsNone(dictionaries.a_dict().get("nonexistent_key"))
        self.assertIsNone(dictionaries.a_dict().get("nonexistent_key", None))

    def test_how_to_get_default_value_when_key_does_not_exist(self):
        self.assertEqual(dictionaries.a_dict().get("nonexistent_key", "me"), "me")

    def test_listing_dictionary_keys(self):
        self.assertEqual(list(dictionaries.a_dict().keys()), ["key"])

    def test_listing_dictionary_items(self):
        self.assertEqual(list(dictionaries.a_dict().items()), [("key", "value")])

    def test_person_dictionary(self):
        self.assertEqual(
            list(dictionaries.person().keys()),
            ["first_name", "last_name", "year_of_birth", "sex", "age"],
        )
        self.assertEqual(
            list(dictionaries.person().values()), ["me", "last_name", "1986", "M", "34"]
        )
        self.assertEqual(
    ...
    def test_set_default_value_to_none_forgiven_key(self):
        ...
        self.assertEqual(test, {"bippity": "boppity", "nonexistent_key": None})

    def test_set_default_value_when_default_value_given(self):
        test = {"bippity": "boppity"}
        test.setdefault("nonexistent_key", "default value")
        self.assertEqual(
            test, {"bippity": "boppity", "nonexistent_key": "default value"}
        )

    def test_set_default_vs_update(self):
        test = {"basic": "crest", "whitening": "colgate"}
    ...
    def test_set_default_vs_update(self):
        test = {"basic": "crest", "whitening": "colgate"}
        test.update(
            {"non-basic": "chewing stick", "browning": "tobacco", "decaying": "sugar"}
        )
        self.assertEqual(
            test,
            {
